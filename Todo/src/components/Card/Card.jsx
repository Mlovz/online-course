...
const Card = ({
  ...
  return (
    <ul className={cls.card} key={id}>
      <li className={cls.todsWrapper}>
        <label
          className={`${isCompleted ? cls.checked : cls.check}`}
          onClick={() => handleIsCompleted(id)}
        ></label>
        <span className={isCompleted ? cls.todoText : cls.text}>{text}</span>
        <button onClick={() => deleteTodo(id)} disabled={!isCompleted}>
          <svg
            width="13"
            height="14"
            viewBox="0 0 13 14"
          ...
          >
            ...
            />
            <path
              d="M5.46233 4.98548H4.13177V10.5073H5.46233V4.98548Z"
              fill="#808080"
            />
            <path
